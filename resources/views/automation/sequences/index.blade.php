...
@section('automation_content')
    <div class="automation-page-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px; margin-bottom: 24px;">
        <div>
            <h2 style="font-size: 18px; margin: 0 0 6px 0; color: var(--theme-sidebar-text, #1E40AF);">Sequences</h2>
            <p style="font-size: 14px; color: #64748B; margin: 0;">Email sequences triggered by events like funnel opt-in.</p>
        </div>
        <a href="{{ route('automation.sequences.create') }}" class="btn-create"><i class="fas fa-plus"></i> Create Sequence</a>
    </div>
    ...
    <div class="actions" style="margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
        <form method="GET" action="{{ route('automation.sequences.index') }}" style="display: flex; gap: 8px; flex-wrap: wrap; align-items: center;">
            <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search sequences..."
    ...
    <div class="card">
                    ...
                    <tbody>
                        @foreach($sequences as $seq)
                            <tr>
                                ...
                                <td>{{ isset($seq['updated']) ? \Carbon\Carbon::parse($seq['updated'])->format('M j, Y') : '—' }}</td>
                                <td class="automation-actions-cell">
                                    <a href="{{ route('automation.sequences.edit', $seq['id']) }}" class="icon-btn" title="Edit"><i class="fas fa-pen"></i></a>
                                    <form method="POST" action="{{ route('automation.sequences.toggle', $seq['id']) }}" style="display: inline;">
                                        @csrf
                                        <button type="submit" class="icon-btn" title="{{ ($seq['status'] ?? '') === 'active' ? 'Pause' : 'Activate' }}"><i class="fas {{ ($seq['status'] ?? '') === 'active' ? 'fa-pause' : 'fa-play' }}"></i></button>
                                    </form>
                                    <form method="POST" action="{{ route('automation.sequences.destroy', $seq['id']) }}" style="display: inline;" onsubmit="return confirm('Delete this sequence?');">
                                        @csrf
                                        @method('DELETE')
        ...
        @else
            <div class="empty-state">
                <div class="empty-state-icon"><i class="fas fa-list-ul"></i></div>
                <h3>No sequences yet</h3>
                <p>Create a sequence to send automated emails. Sequences run when a workflow starts them.</p>
                <a href="{{ route('automation.sequences.create') }}" class="btn-create"><i class="fas fa-plus"></i> Create Sequence</a>
            </div>
        @endif
    </div>
